...
CoCylinder::CoCylinder()
    ...
    , m_fLength(30)
{
    m_eShaderProgramType = EShaderProgramType::eCylinder;

    createCylinder();
//    createIndexedCylinder();
}

CoCylinder::CoCylinder(CoVec3 vCenter,
                       Gfloat fRadius,
                       Gfloat fLength)
    : m_vCenter(vCenter)
    , m_fRadius(fRadius)
...
/**
 * @brief This function is used to opengl coordinate opengl to create vertice of cylinder.
 *        z-axis is screen.
 *        y-axis is up.
 *        x-aixs is right.
 */
void CoCylinder::createCylinder()
{
 ...
 *        x-aixs is right.
 */
void CoCylinder::createIndexedCylinder()
{

}

void CoCylinder::setCenter(CoVec3 vPosition)
{
    m_vCenter = vPosition;
}

void CoCylinder::setRadius(Gfloat fRadius)
{
    m_fRadius = fRadius;
}

void CoCylinder::setLength(Gfloat fLength)
{
    m_fLength = fLength;
}

std::vector<CoVec3> CoCylinder::getNormals()
{
    return m_vecVertexNormal;
}

std::vector<Gint> CoCylinder::getVertexIndice()
{
